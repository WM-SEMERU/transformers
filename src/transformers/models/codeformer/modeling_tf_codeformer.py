...
from .configuration_codeformer import CodeformerConfig


logger = logging.get_logger(__name__)

_CHECKPOINT_FOR_DOC = "EleutherAI/gpt-j-6B"
_CONFIG_FOR_DOC = "CodeformerConfig"
_TOKENIZER_FOR_DOC = "GPTJTokenizer"

CODEFORMER_PRETRAINED_MODEL_ARCHIVE_LIST = [
    "EleutherAI/gpt-j-6B",
    # See all codeformer models at https://huggingface.co/models?filter=codeformer
]
...
# Copied from transformers.models.gptj.modeling_tf_gptj.TFGPTJAttention with GPTJ->Codeformer
class TFCodeformerAttention(tf.keras.layers.Layer):
    def __init__(self, config: CodeformerConfig, **kwargs):
...
# Copied from transformers.models.gptj.modeling_tf_gptj.TFGPTJMLP with GPTJ->Codeformer
class TFCodeformerMLP(tf.keras.layers.Layer):
    def __init__(self, intermediate_size: int, config: CodeformerConfig, **kwargs):
        super().__init__(**kwargs)
...
# Copied from transformers.models.gptj.modeling_tf_gptj.TFGPTJBlock with GPTJ->Codeformer
class TFCodeformerBlock(tf.keras.layers.Layer):
    def __init__(self, config: CodeformerConfig, **kwargs):
        super().__init__(**kwargs)
        inner_dim = config.n_inner if config.n_inner is not None else 4 * config.n_embd
        self.ln_1 = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_epsilon, name="ln_1")
        self.attn = TFCodeformerAttention(config, name="attn")
        self.mlp = TFCodeformerMLP(inner_dim, config, name="mlp")
...
@keras_serializable
# Copied from transformers.models.gptj.modeling_tf_gptj.TFGPTJMainLayer with GPTJ->Codeformer
class TFCodeformerMainLayer(tf.keras.layers.Layer):
    config_class = CodeformerConfig

    def __init__(self, config: CodeformerConfig, *inputs, **kwargs):
        ...
        self.h = [TFCodeformerBlock(config, name=f"h_._{i}") for i in range(config.n_layer)]
...
# Copied from transformers.models.gptj.modeling_tf_gptj.TFGPTJPreTrainedModel with GPTJ->Codeformer
class TFCodeformerPreTrainedModel(TFPreTrainedModel):
    """
...
CODEFORMER_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`CodeformerConfig`]): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the [`~TFPreTrainedModel.from_pretrained`] method to load the model weights.
"""

CODEFORMER_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`Numpy array` or `tf.Tensor` of shape `(batch_size, input_ids_length)`):
            ...
            Indices can be obtained using [`CodeformerTokenizer`]. See [`PreTrainedTokenizer.__call__`] and
...
@add_start_docstrings(
    "The bare codeformer Model transformer outputting raw hidden-states without any specific head on top.",
    CODEFORMER_START_DOCSTRING,
)
class TFCodeformerModel(TFCodeformerPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFCodeformerMainLayer(config, name="transformer")

    @unpack_inputs
    @add_start_docstrings_to_model_forward(CODEFORMER_INPUTS_DOCSTRING)
    @add_code_sample_docstrings(
...
@add_start_docstrings(
    """
    The codeformer Model transformer with a language modeling head on top.
    """,
    CODEFORMER_START_DOCSTRING,
)
class TFCodeformerForCausalLM(TFCodeformerPreTrainedModel, TFCausalLanguageModelingLoss):
    def __init__(self, config, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        self.transformer = TFCodeformerMainLayer(config, name="transformer")
    ...
    @unpack_inputs
    @add_start_docstrings_to_model_forward(CODEFORMER_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @add_code_sample_docstrings(
        processor_class=_TOKENIZER_FOR_DOC,
        checkpoint=_CHECKPOINT_FOR_DOC,
...
@add_start_docstrings(
    """
    The codeformer Model transformer with a sequence classification head on top (linear layer).

    [`CodeformerForSequenceClassification`] uses the last token in order to do the classification, as other causal models
...
class TFCodeformerForSequenceClassification(TFCodeformerPreTrainedModel, TFSequenceClassificationLoss):
...
@add_start_docstrings(
    """
    The codeformer Model transformer with a span classification head on top for extractive question-answering tasks like
    SQuAD (a linear layers on top of the hidden-states output to compute `span start logits` and `span end logits`).
    """,
    CODEFORMER_START_DOCSTRING,
)
class TFCodeformerForQuestionAnswering(TFCodeformerPreTrainedModel, TFQuestionAnsweringLoss):
    _keys_to_ignore_on_load_missing = [r"h.\d+.attn.masked_bias", r"h.\d+.attn.bias", r"lm_head.weight"]
